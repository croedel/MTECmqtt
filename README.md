...
| current_PV                  | float | W    | Current power generated by PV 
| current_grid                | float | W    | Current power flow to/from grid (flow to grid is represented by pos. values)
| current_battery             | float | W    | Current power flow to/from battery (flow from battery is represented by pos. values)
| current_inverter            | float | W    | Current power flow to/from inverter (flow from inverter is represented by pos. values)
| current_backup              | float | W    | Current backup power flow
| current_consumption         | float | W    | Current consumption 
| current_battery_SOC         | int   | %    | Current battery SOC
| day_PV                      | float | kWh  | Energy generated PV today 
| day_grid_feed               | float | kWh  | Energy feed into grid today
| day_grid_purchase           | float | kWh  | Energy purchased from grid today
| day_battery_charge          | float | kWh  | Energy charged to battery today
| day_battery_discharge       | float | kWh  | Energy discharged from battery today
| day_consumption             | float | kWh  | Own energy consumption today (*)
| day_autarky_rate            | float | %    | Autarky rate today (*)
| day_own_consumption_rate    | float | %    | Own consumption rate today (*)
| total_PV                    | float | kWh  | Energy generated PV 
| total_grid_feed             | float | kWh  | Energy feed into grid 
| total_grid_purchase         | float | kWh  | Energy purchased from grid 
| total_battery_charge        | float | kWh  | Energy charged to battery 
| total_battery_discharge     | float | kWh  | Energy discharged from battery 
| total_consumption           | float | kWh  | Own energy consumption (*)
| total_autarky_rate          | float | %    | Autarky rate (*)
| total_own_consumption_rate  | float | %    | Own consumption rate (*)
