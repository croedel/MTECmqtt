...
def get_register_list( type ):
  if type == "config":
    ...
    logging.error("Unknown read type: {}".format(type))
    return None              
  return registers

# read data from MTEC modbus
def read_MTEC_data( type ):
  logging.info("Reading registers for type: {}".format(type))
  registers = get_register_list( type )
  now = datetime.now()
